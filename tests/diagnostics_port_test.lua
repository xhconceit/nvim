...
local native_diagnostics = require("nvi.adapters.native_diagnostics")

assert(
  DiagnosticsPort.validate(native_diagnostics) == native_diagnostics,
...
assert(not ok, "缺少 open_list 时应该验证失败")
...
assert(not workspace_ok, "缺少 open_workspace_list 时应该验证失败")
